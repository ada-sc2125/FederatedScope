...
class Server(object):
    def __init__(self, args, eval_loader, candidate_seeds, log_dir):
        ...
        if self.args.bias_sampling:
            ...
            self.gradient_history = {
                seed: [self.args.grad_initial] for seed in self.candidate_seeds
            }
            self.probabilities = [
                1.0 / float(len(self.candidate_seeds))
                for _ in range(len(self.candidate_seeds))
            ]
        else:
    ...
    def create_model_by_seedpool(self, cur_round):
        ...
        framework = MeZOFramework(
            tmp_model, args=self.args, lr=lr, candidate_seeds=self.candidate_seeds
        )
    ...
    def aggregate_seed_pool(self, selected_client_list, client_adj):
        """
        Performs decentralized aggregation of seed pools for each client.
        For each client, it averages its seed pool with those of its neighbors.
        """
        new_client_seed_pools = {}
        client_seed_pools = {
            client.idx: client.local_seed_pool for client in selected_client_list
        }

        for client in selected_client_list:
            client_id = client.idx
            # Assuming client_adj is a dict {client_id: [neighbor_ids]}
            neighbor_ids = client_adj.get(client_id, [])
            ids_to_aggregate = [client_id] + neighbor_ids

            # Simple average for weighting
            num_to_aggregate = 0

            aggregated_seed_pool = {seed: 0.0 for seed in self.candidate_seeds}

            for neighbor_id in ids_to_aggregate:
                if neighbor_id in client_seed_pools:
                    num_to_aggregate += 1
                    neighbor_pool = client_seed_pools[neighbor_id]
                    for seed, value in neighbor_pool.items():
                        aggregated_seed_pool[seed] += value

            if num_to_aggregate > 0:
                for seed in aggregated_seed_pool:
                    aggregated_seed_pool[seed] /= num_to_aggregate

            new_client_seed_pools[client_id] = aggregated_seed_pool

        # After computing all new pools, update the clients
        for client in selected_client_list:
            client.local_seed_pool = new_client_seed_pools[client.idx]
    ...
    def online_aggregate(self, client, selected_client_list):
        if self.args.equal_weight:
            weight_array = np.array(
                [1.0 for _ in selected_client_list], dtype=np.float64
            )
            weight_array /= float(len(selected_client_list))
        else:
            weight_array = np.array(
                [len(client.train_loader) for client in selected_client_list],
                dtype=np.float64,
            )
    ...
    def eval(self, cur_round, eval_avg_acc):
        ...
        if self.args.save and cur_round > 0:
            ...
            if (
                self.args.eval_metric == "loss" and eval_metric < np.min(eval_avg_acc)
            ) or (
                ...
                torch.save(
                    ...
                    os.path.join(
                        save_dir, f"model_state_dict_best_round{cur_round}.bin"
                    ),
                )
            for file_name in os.listdir(save_dir):
            ...
            torch.save(
                self.model.state_dict(),
                os.path.join(save_dir, f"model_state_dict_final_round{cur_round}.bin"),
            )
        return eval_metric
    ...
    def eval_loss(self, cur_round):
        ...
        if num_eval == 0:
            print(
                "Warning: num_eval is 0 in eval_loss. All evaluation batches might have resulted in NaN loss."
            )
            return float("inf")
